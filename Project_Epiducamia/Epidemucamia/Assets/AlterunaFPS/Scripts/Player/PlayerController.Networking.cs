...
namespace AlterunaFPS
{
	...
	private void InitializeNetworking()
	{
		...
		else
		{
			...
			_isHost = Avatar.Possessor != null && Avatar.Possessor.IsHost;
			...
			// If Avatar exists but is not possessed (offline/single-player), initialize as owner
			if (Avatar.IsPossessed)
			{
				OnPossession();
			}
			else if (Multiplayer == null || Multiplayer.InRoom == false)
			{
				// No multiplayer session active - treat as offline
				_isOwner = true;
				_offline = true;
				OnPossession();
			}
