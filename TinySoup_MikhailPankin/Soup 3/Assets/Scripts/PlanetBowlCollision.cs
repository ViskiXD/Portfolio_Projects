...
public class PlanetBowlCollision : MonoBehaviour
{
    ...
    [SerializeField] private bool enableCollisionAudio = true;
    ...
    void Start()
    {
        ...
        if (audioSource == null && enableCollisionAudio)
        {
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.gameObject == bowlObject)
        {
            ...
            if (enableExplosions)
            {
                SpawnExplosion(collisionPoint, contact.normal);
            }
            
            // Play collision sound
            if (enableCollisionAudio)
            {
                PlayCollisionSound();
            }
            
            // Apply bounce effect
            Vector3 bounceVelocity = Vector3.Reflect(planetRigidbody.linearVelocity, contact.normal);
        ...
        // Also handle planet-to-planet collisions
        if (collision.gameObject.GetComponent<PlanetBowlCollision>() != null)
        {
            ContactPoint contact = collision.contacts[0];
            Vector3 collisionPoint = contact.point;
            
            // Spawn explosion at collision point between planets
            if (enableExplosions)
            {
                SpawnExplosion(collisionPoint, contact.normal);
            }
    ...
    void SpawnExplosion(Vector3 position, Vector3 normal)
    {
        ...
        // Pick random explosion from array
        GameObject explosionPrefab = explosionPrefabs[Random.Range(0, explosionPrefabs.Length)];
        
        if (explosionPrefab != null)
        {
            // Spawn explosion at collision point
            GameObject explosion = Instantiate(explosionPrefab, position, Quaternion.LookRotation(normal));
            
            // Scale the explosion
            explosion.transform.localScale = Vector3.one * explosionScale;
            
            // Add animator component if not present to enable sprite sheet animation
            if (explosion.GetComponent<ExplosionAnimator>() == null)
            {
                explosion.AddComponent<ExplosionAnimator>();
            }
    ...
    void PlayCollisionSound()
    {
        ...
        if (clip != null)
        {
            ...
            audioSource.pitch = audioPitch + Random.Range(-audioPitchVariation, audioPitchVariation);
