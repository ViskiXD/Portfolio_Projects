...
public class PlanetAudio : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        // Only add RNBO processing if the plugin is available
        // We check if SpaceEchoHandle exists by seeing if the type can be found
        if (GetComponent<RNBOAudioFilter>() == null)
        {
            // Only add filter if user specifically wants it (disabled by default since plugin is missing)
