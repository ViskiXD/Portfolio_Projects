...
public class SpaceEchoTiltController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Try to create SpaceEcho handle - may fail if native plugin is missing
        try
        {
            ...
            Debug.Log("✅ SpaceEcho initialized successfully!");
        }
        catch (System.DllNotFoundException)
        {
            Debug.Log("ℹ️ SpaceEcho native plugin not found - using basic audio instead. This is normal.");
            handle = null;
            this.enabled = false; // Disable this component since plugin isn't available
            return;
        }
        catch (System.Exception e)
        {
            Debug.LogWarning($"Failed to initialize SpaceEcho: {e.Message}");
            handle = null;
            this.enabled = false;
            return;
        }
    ...
    void Update()
    {
        ...
        if (handle != null)
        {
            ...
            {
                Debug.LogWarning($"SpaceEcho Update failed: {e.Message}");
            }
        }
